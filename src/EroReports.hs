  ...
  if exists
    then pure ()
      ...
      r <- req GET
               ...
               (NoReqBody)
               bsResponse
               (queryParam "id" (pure schoolNumber) <> nzhScraper)
      liftIO
        $ T.writeFile p (T.decodeUtf8With (\_ _ -> Nothing) $ responseBody r)
...
allReports outfile files = do
 ...
 where
  go fp = do
    bdy <- T.readFile fp
    let links =
          map (\l -> "https://www.ero.govt.nz" <> l)
            $ maybe [] nub
            $ scrape tagLink
            $ parseTags bdy
        dates = map show $ catMaybes $ map
          (extractDate . T.takeEnd 10 . T.replace "/" "")
          links
        sn = takeBaseName fp
...
report fn = do
  ...
  if exists
    then print ("exists " ++ fn)
      ...
      let l = reportLinkUrl $ V.head $ V.filter
            (\(ReportLink { reportLinkSchoolNumber, reportLinkDate }) ->
              reportLinkSchoolNumber == sn && reportLinkDate == d
            )
            reps
